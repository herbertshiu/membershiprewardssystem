# Article generation prompt (domain 1)

Edit the fenced blocks below. `generate-page.js` reads them automatically on each run.
...
- Language: formal written Traditional Chinese (書面語). Use complete sentences; avoid colloquial particles (唔、咗、嚟、揀、睇), slang, or intern-style casual voice.
...
- SEO: weave keywords naturally from the topic brief. Do NOT keyword-stuff or use <strong> for SEO.
...
- Use today's date {{TODAY}} in <time datetime="YYYY-MM-DD"> with visible text as YYYY年M月D日 (no leading zero on month/day).
